...
const applicationSlice = createSlice({
  ...
  reducers: {
    requestForAllApplications(state, action) { //for employer
      state.loading = true;
      state.error = null;
    },
    ...
    requestForMyApplications(state, action) {   //for job seeker
      state.loading = true;
      state.error = null;
    },
    ...
    successForPostApplication(state, action) {
      state.loading = false;
      state.error = null;
      state.message = action.payload;   //message me success message hi aata h
    },
...
export const postApplication = (data, jobId) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `http://localhost:4000/api/v1/application/post/${jobId}`, //jobId dynamic.
      data,
      {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" }, //multipart isiliye kiya jo data bhejenge usme file(resume) bhi hogi.
      }
    );
    dispatch(
