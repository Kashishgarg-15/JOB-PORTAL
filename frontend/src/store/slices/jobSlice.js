...
const jobSlice = createSlice({
  name: "jobs",
  initialState: {
    jobs: [],   // List of all jobs fetched
    loading: false,   // Whether a fetch request is ongoing
    error: null,   // Holds any error message if the fetch fails
    message: null,   // Could hold success or info messages?
    singleJob: {},    // Details for one specific job?
    myJobs: [],   // Jobs specific to the current user?
  },
  reducers: {
    ...
    clearAllErrors(state, action) {
      state.error = null;
      state.jobs = state.jobs;   //unnecessary 
    },
    // initial state aani chahiye na reset krne k baad to see?
...
export const fetchJobs =
  (city, niche, searchKeyword = "") =>    //()=>() 1st bracket me agar frontend se kuch bhejna h to vo likhte h.
  async (dispatch) => {                   // and 2nd bracket me dispatch likhte h and agar uske andar koi async function likhenge to "async"
    try {                                  // bhi lga dete h.
